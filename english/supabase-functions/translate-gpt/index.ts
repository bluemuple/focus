...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const sys = [
      ...
      `a Korean learner. The "Sentence" field is the surrounding sentence —`,
      `use it to pick the SYNTACTIC ROLE the phrase plays so the Korean`,
      `output reads naturally inside that sentence.`,
      "",
      "Return JSON ONLY (no markdown, no quotes around value):",
      "  { translation: string }",
      "",
      "Rules:",
      "- Translate the Word/Phrase ONLY. The output should be the Korean",
      "  rendering of the SAME word/phrase as it functions in the given",
      "  sentence. Length matches the phrase: a single word → 1-6 syllables,",
      "  a multi-word phrase → as many syllables as needed (typically",
      "  6-20). NEVER return a translation of the whole Sentence.",
      ...
      source === "JA"
        ...
        : [
            ...
            '- SYNTACTIC ROLE matters when the phrase is multi-word:',
            '  • TO-infinitive after a NOUN (modifies it as a relative clause):',
            '    use the Korean adnominal ending -는 / -할 / -해야 하는 /',
            '    -하려는 — match what the surrounding noun expects.',
            '    e.g. "She felt pressure to optimize their children for success."',
            '         "to optimize their children" → "자녀를 최적화해야 하는"',
            '    e.g. "The decision to leave was final."',
            '         "to leave" → "떠나기로 한"',
            '  • TO-infinitive of PURPOSE (in order to …): -기 위해 / -하려고.',
            '    e.g. "She left to find peace." → "to find peace" → "평화를 찾기 위해"',
            '  • Bare -ING gerund used as a noun: -는 것 / -기.',
            '    e.g. "running marathons" → "마라톤을 뛰는 것"',
            '  • -ING / -ED participle modifying a noun: relative clause,',
            '    -는/-한 ending agreeing with the noun.',
            '    e.g. "the man standing there" → "거기 서 있는 (남자)"',
            '  • Prepositional phrase modifying a noun: -의 / 에 있는 / 에 대한 / -하는, etc.',
            '    e.g. "the book on the table" → "테이블 위에 있는 (책)"',
            '  • Bare verb phrase as predicate: standard predicate Korean,',
            '    matching the sentence\'s tense / aspect.',
            '- Do NOT include the surrounding noun in the output — return',
            '  ONLY the modifier portion. e.g. for "pressure to optimize",',
            '  return "최적화해야 하는" (the modifier of pressure), not',
            '  "최적화해야 하는 압력".',
          ].join("\n"),
      "",
      "EXAMPLE",
      ...
      "",
      "EXAMPLE (EN — to-infinitive after noun, full sentence context)",
      'Sentence: "She felt pressure to optimize their children for success."',
      'Word/Phrase: "to optimize their children"',
      'Output: { "translation": "자녀를 최적화해야 하는" }',
      "",
    ...
    const r = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: "gpt-4o-mini",
        temperature: 0.2,
        // 200 leaves room for multi-word phrase translations like
        // "자녀를 최적화해야 하는" (Korean adnominal modifiers can run
        // 8-20 syllables) without truncation. Single-word output still
        // uses well under 50 tokens.
        max_tokens: 200,
