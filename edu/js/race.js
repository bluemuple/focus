...
(() => {
  ...
  // PER QUESTION to the upper bound of that range.
  const SPEEDS = {
    1: { beg: [36, 70], int: [10, 35], adv: [1, 9]  },
    2: { beg: [40, 76], int: [12, 39], adv: [1, 11] },
    3: { beg: [44, 82], int: [14, 43], adv: [1, 13] },
    4: { beg: [48, 88], int: [16, 47], adv: [1, 15] },
    5: { beg: [52, 94], int: [18, 51], adv: [1, 17] },
    6: { beg: [56,100], int: [20, 55], adv: [1, 19] },
  };
  const SPEED_LABELS = { beg: 'Beginner', int: 'Intermediate', adv: 'Advanced' };
        ...
        <div class="rc-speed-grid">
          ${['adv','int','beg'].map(s => {
            ...
            return `<button class="rc-speed ${s}" data-s="${s}">
                      ...
                      <span class="rng">${r[0]}–${r[1]}s per Q</span>
                    </button>`;
          }).join('')}
        </div>
      </div>
  ...
  // ---- Score + persistence ---------------------------------------
  function calcScore(correct, avgTimeSec) {
    ...
    return { correctPts, timeBonus, total: correctPts + timeBonus };
  }
  async function finishRace() {
    const avgTime = timeTotal / QUESTIONS_PER_RACE;
    const { correctPts, timeBonus, total } = calcScore(correctCount, avgTime);
    ...
    $('card').innerHTML = `
      <div class="rc-summary">
        ...
        <div style="font-size:13px; color:#6b7280; margin: 6px 0 0;">
          Score breakdown: ${correctPts} (correct) + ${timeBonus} (time bonus) = ${total}
        </div>
        <div class="rc-actions">
